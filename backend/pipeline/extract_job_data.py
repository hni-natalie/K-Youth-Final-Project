...
def process_batch_jobs(job_queue: list, block_dir: Path) -> tuple[int, int, list]:
    saved_job_blocks = 0
    non_tech_skipped = 0
    skipped_job_titles = []
    ...
    for i in range(0, len(job_queue), BATCH_SIZE):
        ...
        if "Error" in ai_res:
            print(f"⚠️ AI Service Error (skipping classification): {ai_res}")
        ...
        for is_tech, job in zip(tech_list, batch):
            ...
            block_file = block_dir / f"{job_id}_{safe_title}.html"
