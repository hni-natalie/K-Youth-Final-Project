...
def save_new_tech_jobs(job_queue, existing_job_ids):
    ...
    def filter_fn(job_id, title, block, is_tech):
        ...
        # ✅ SAFE WRITE (fixes your crash)
        safe_write(file_path, str(block))
...
def main():
    ...
    saved, skipped_non_tech, skipped_non_tech_titles = save_new_tech_jobs(
        job_queue,
        existing_job_ids
    )
    ...
    print("\n🧹 Cleaning job_sources folder...")
    try:
        shutil.rmtree(HTML_DIR)
        print(f"🗑️ Removed folder: {HTML_DIR}")
    except Exception as e:
        print(f"⚠️ Failed to remove job_sources: {e}")
